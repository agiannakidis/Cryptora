...
router.get('/summary', requirePartnerAuth, async (req, res) => {
  try {
    ...
    const [stats,clicks,earned] = await Promise.all([
      queryOne(`SELECT COUNT(DISTINCT referred_user_id)::int as regs,COUNT(DISTINCT CASE WHEN first_deposit_at IS NOT NULL THEN referred_user_id END)::int as ftds,COALESCE(SUM(first_deposit_amount),0)::float as deps FROM affiliate_referrals WHERE affiliate_id=$1 AND created_date::date BETWEEN $2 AND $3`,[aff.id,from,to]),
...
router.get('/timeseries', requirePartnerAuth, async (req, res) => {
  try {
    ...
    const from = req.query.from||new Date(Date.now()-30*864e5).toISOString().slice(0,10);
    const to   = req.query.to  ||new Date().toISOString().slice(0,10);
    const rows = await queryAll(`SELECT date_trunc('day',created_date)::date as date,COUNT(DISTINCT referred_user_id)::int as registrations,COUNT(DISTINCT CASE WHEN first_deposit_at IS NOT NULL THEN referred_user_id END)::int as ftds FROM affiliate_referrals WHERE affiliate_id=$1 AND created_date::date BETWEEN $2 AND $3 GROUP BY 1 ORDER BY 1`,[aff.id,from,to]);
