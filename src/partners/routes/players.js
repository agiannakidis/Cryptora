...
router.get('/', requirePartnerAuth, async (req, res) => {
  try {
    ...
    const rows=await queryAll(`SELECT ar.referred_user_id as id,ar.referred_user_email,ar.status,ar.created_date as registered_at,ar.first_deposit_date,ar.first_deposit_amount FROM affiliate_referrals ar WHERE ar.affiliate_id=$1${w} ORDER BY ar.created_date DESC LIMIT $2 OFFSET $3`,params);
    const cnt=await queryOne(`SELECT COUNT(*)::int as n FROM affiliate_referrals WHERE affiliate_id=$1`,[aff.id]);
    res.json({ players:rows.map(r=>({...r,referred_user_email:r.referred_user_email?r.referred_user_email.replace(/^(.{2}).*(@.*)$/,'$1***$2'):'—',first_deposit_amount:parseFloat(r.first_deposit_amount||0), sub1:null, sub2:null})), total:cnt?.n||0, limit, offset });
