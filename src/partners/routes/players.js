...
router.get('/', requirePartnerAuth, async (req, res) => {
  try {
    ...
    const rows=await queryAll(`SELECT ar.referred_user_id as id,ar.referred_user_email,ar.status,ar.created_date as registered_at,ar.first_deposit_date,ar.first_deposit_amount,ar.sub1,ar.sub2 FROM affiliate_referrals ar WHERE ar.affiliate_id=$1${w} ORDER BY ar.created_date DESC LIMIT $2 OFFSET $3`,params);
