...
router.get('/', requirePartnerAuth, async (req, res) => {
  try {
    ...
    const rows=await queryAll('SELECT id,amount,status,period_start,period_end,revshare_percent,total_ggr as ngr,created_at,paid_at FROM affiliate_commissions WHERE affiliate_id=$1 ORDER BY created_at DESC LIMIT $2 OFFSET $3',[aff.id,limit,offset]);
