...
router.get('/performance', requirePartnerAuth, async (req,res) => {
  try {
    ...
    const rows=await queryAll(`SELECT created_date::date as date,sub1,sub2,COUNT(DISTINCT referred_user_id)::int as registrations,COUNT(DISTINCT CASE WHEN first_deposit_at IS NOT NULL THEN referred_user_id END)::int as ftds,COALESCE(SUM(first_deposit_amount),0)::float as deposits FROM affiliate_referrals WHERE affiliate_id=$1 AND created_date::date BETWEEN $2 AND $3 GROUP BY 1,2,3 ORDER BY 1 DESC LIMIT $4 OFFSET $5`,[aff.id,from,to,limit,offset]);
