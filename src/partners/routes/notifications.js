...
router.get('/', requirePartnerAuth, async (req,res) => {
  try {
    ...
    const ftds=await queryAll('SELECT referred_user_id,first_deposit_at,first_deposit_amount FROM affiliate_referrals WHERE affiliate_id=$1 AND first_deposit_at IS NOT NULL ORDER BY first_deposit_at DESC LIMIT 3',[aff.id]);
    for(const r of ftds) notifs.push({id:'ftd_'+r.referred_user_id,type:'new_ftd',title:'New First Deposit',message:'$'+parseFloat(r.first_deposit_amount||0).toFixed(2)+' first deposit',created_at:r.first_deposit_at,read:false});
