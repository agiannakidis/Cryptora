...
router.get('/', requirePartnerAuth, async (req,res) => {
  try {
    ...
    const ftds=await queryAll('SELECT referred_user_id,first_deposit_date,first_deposit_amount FROM affiliate_referrals WHERE affiliate_id=$1 AND first_deposit_date IS NOT NULL ORDER BY first_deposit_date DESC LIMIT 3',[aff.id]);
